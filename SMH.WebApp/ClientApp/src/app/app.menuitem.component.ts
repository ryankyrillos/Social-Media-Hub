		...
		<ng-container>
			...
			<a (click)="itemClick($event)" *ngIf="(item.routerLink && !item.items) && item.visible !== false" [ngClass]="item.class"
			   ...
			   [routerLinkActiveOptions]="{exact: !item.preventExact}" [attr.target]="item.target" [attr.tabindex]="0" [attr.aria-label]="item.label" role="menuitem" pRipple>
                <div *ngIf="isTiktok; else elseBlock"><fa-icon [icon]="faTikTok" class="layout-menuitem-icon"></fa-icon></div>
                <ng-template #elseBlock><i [ngClass]="item.icon" class="layout-menuitem-icon"></i></ng-template>
				<span>{{item.label}}</span>
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.isTiktok = this.item.icon === "tiktok";
    }
